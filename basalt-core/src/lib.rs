...
pub fn remove_empty_folder(root: &Path, rel: String) -> Result<(), String> {
    let r = rel.trim().trim_matches(['/', '\\']);
    if r.is_empty() {
...
pub fn list_foreign_files(root: &Path, rel: String) -> Result<Vec<String>, String> {
    let r = rel.trim().trim_matches(['/', '\\']);
    if r.is_empty() {
        return Err("invalid folder path".into());
    }
...
pub fn list_subfolders(root: &Path, rel: String) -> Result<Vec<String>, String> {
    let r = rel.trim().trim_matches(['/', '\\']);
    if r.is_empty() {
        return Err("invalid folder path".into());
    }
...
pub fn rename_folder(root: &Path, from_rel: String, to_rel: String) -> Result<String, String> {
...
pub fn read_obsidian_config(root: &Path) -> Result<ObsidianConfig, String> {
...
pub fn read_obsidian_import(root: &Path) -> Result<ObsidianImport, String> {
...
pub fn read_obsidian_bookmarks(root: &Path) -> Result<Vec<Bookmark>, String> {
...
/// Classify a filesystem watch event into changed Markdown notes plus whether a
/// full rescan is needed — the pure core of the vault watcher. `metadata_only`
/// events (mtime/xattr/ownership; iCloud emits them constantly) change nothing.
/// Otherwise each path is made vault-relative (ignored components skipped), `.md`
/// files become ChangedNotes, and directory-level (no-extension) or attachment
/// paths trigger a rescan.
pub fn classify_change(root: &Path, metadata_only: bool, paths: &[PathBuf]) -> (Vec<ChangedNote>, bool) {
    ...
    for p in paths {
        let Some(rel) = rel_under(root, p) else {
            continue;
        };
...
pub fn list_plugins(root: &Path) -> Result<Vec<PluginInfo>, String> {
...
pub fn write_plugin_data(root: &Path, id: String, data: String) -> Result<(), String> {
    if !valid_plugin_id(&id) {
